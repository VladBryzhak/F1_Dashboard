      ...
      {!loading && data && (
        <div className="card">
          <table className="standings">
            ...
            <tbody>
              {data.races.map((r) => {
                const done = !!r.winnerName
                return (
                  <tr
                    key={r.round}
                    className={done ? 'row-clickable' : undefined}
                    onClick={
                      done
                        ? () => navigate(`/calendar/${season}/${r.round}`)
                        : undefined
                    }
                    title={done ? `View ${r.name} weekend` : undefined}
                  >
                    <td>
                    ...
                    <td className="muted chevron">{done ? '›' : ''}</td>
                  </tr>
                )
              })}
            </tbody>
